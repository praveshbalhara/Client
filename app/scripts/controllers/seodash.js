...
client.controller('SEODashCtrl', function ($scope, myutil) {
    ...
    $scope.assignedToFilter = '';
     myutil.getAllRequest(0,1).then(function(data){
    ...
    $scope.refreshData = function(){
        clearMatchValues($scope, myutil);
        myutil.getAllRequest(1,1).then(function(data){
            debugger;
            $scope.report = data.report;
            $scope.count = data.count;
            $scope.pageStart = data.pageStart;
