...
client.controller('AllCtrl', function ($scope, myutil) {
    ...
    myutil.getAllRequest(0,'all').then(function(data){
    ...
    $scope.searchRequester = function(){
        $scope.isProcessing = true;
        myutil.getdataByMatching('Requester', $scope.requester);
        setValues($scope, myutil);
    }
    $scope.refreshOpenData = function(){
        clearMatchValues($scope, myutil);
        myutil.getAllRequest(0,'all').then(function(data){
            $scope.report = data.report;
            $scope.count = data.count;
    ...
    $scope.refreshData = function(){
        clearMatchValues($scope, myutil);
        myutil.getAllRequest(1,'all').then(function(data){
            $scope.report = data.report;
            $scope.count = data.count;
            $scope.pageStart = data.pageStart;
            $scope.pageEnd = data.pageEnd;
